			...
			<div class="box-body table-responsive no-padding">
				<table class="table table-hover">
					<tbody>
						...
						@foreach($tags as $tag)
						<tr id="tr-{{$tag->id}}" class="fila">
							<td>{{$tag->id}}</td>
							<td>{{$tag->name}}</td>
							<td>{{$tag->slug}}</td>
							<td>
								<a href="#" name="{{$tag->id}}" class="editar"><span class="label label-info">Editar</span></a>
								<a href="#" name="{{$tag->id}}" class="eliminar"><span class="label label-danger">Eliminar</span></a>
							</td>
						</tr>
						@endforeach
					</tbody>
...
<script>
	...
	$(function () {
		...
		$('.eliminar').click(function () {
			...
			.then((willDelete) => {
				if (willDelete) {
					eliminarEtiqueta(id_boton);
				}
			});
		});

		function editarEtiqueta(id_boton){

			$.ajax({
				type: "POST",
				url: '{{ action('TagController@editarRegistro')}}',
		...
		function agregarEtiqueta(){
			...
			$.ajax({
				type: "POST",
				url: '{{ action('TagController@agregarRegistro')}}',
				data: $('#formulario').serialize(),
				success: function (data) {
					swal({
						title: ":D",
		...
		function eliminarEtiqueta(id_boton){

			$.ajax({
				type: "POST",
				url: '{{ action('TagController@eliminar')}}',
				data:{
					"id" : id_boton
				},
				success: function (data) {
